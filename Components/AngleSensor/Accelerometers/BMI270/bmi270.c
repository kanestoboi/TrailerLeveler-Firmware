...
const uint8_t bmi270_config_file[] = {BMI270_INIT_DATA_REG,
...
bool bmi270_register_write(BMI270 *sensor, uint8_t register_address, uint8_t value)
{
    ret_code_t err_code;
    uint8_t tx_buf[BMI270_ADDRESS_LEN+1];
...
bool bmi270_write_bit(BMI270 *sensor, uint8_t register_address, uint8_t bitNumber, uint8_t data)
{
  ...
  bmi270_register_read(sensor, register_address, &registerValue, 1);

  registerValue = (data != 0) ? (registerValue | (1 << bitNumber)) : (registerValue & ~(1 << bitNumber));

  return bmi270_register_write(sensor, register_address, registerValue);
}

bool bmi270_read_bit(BMI270 *sensor, uint8_t register_address, uint8_t bitNumber, uint8_t *data)
{
  uint8_t registerValue;
  bool readSuccess = bmi270_register_read(sensor, register_address, &registerValue, 1);
...
{   
  ...
  if (bmi270_VerifyChipId(sensor) == false)
  {
    return false;
  }
...
bool bmi270_ReadAcc(BMI270 *sensor, int16_t *pACC_X , int16_t *pACC_Y , int16_t *pACC_Z )
{
  ...
  if(bmi270_register_read(sensor, BMI270_DATA_8_REG, buf, 6) == true)
  {
...
bool bmi270_ReadTemp(BMI270 *sensor, float *pTemp )
{
  ...
  if(bmi270_register_read(sensor, BMI270_TEMPERATURE_0_REG, (uint8_t*)&buf, 2) == true)
  {
    
    *pTemp = 23.0 + (float)(buf) * powf(0.5f,9.0f);
...
bool bmi270_VerifyChipId(BMI270 *sensor)
{
  ...
  if (!bmi270_register_read(sensor, BMI270_CHIP_ID_REG, &readId, 1))
  {
    return false;
  }
  
  if (readId != BMI270_CHIP_ID)
  {
    return false;
  }
...
{
    uint8_t pwrControlVal; // create a variable to hold the power configuration value

    if (!bmi270_register_read(sensor, BMI270_PWR_CTRL_REG, &pwrControlVal, 1))
    {
    ...
    if (enable)
    {
        // Set the third bit (bit 2) of pwrConfigVal to 1 to enable the accelerometer
        pwrControlVal = pwrControlVal | (1 << 2);
        bmi270_register_write(sensor, BMI270_PWR_CTRL_REG, pwrControlVal);
    }
    else
    {
        // Set the third bit (bit 2) of pwrConfigVal to 0 to disable the accelerometer
        pwrControlVal = pwrControlVal & ~(1 << 2);
        bmi270_register_write(sensor, BMI270_PWR_CTRL_REG, pwrControlVal);
    }
...
{
    uint8_t pwrConfigVal; // create a variable to hold the power configuration value

    if (!bmi270_register_read(sensor, BMI270_PWR_CONF_REG, &pwrConfigVal, 1))
    {
        return false; // If reading fails, return false
    }
    ...
    {
        // Set the third bit (bit 2) of pwrConfigVal to 1 to enable the accelerometer
        pwrConfigVal = pwrConfigVal | (1 << 0);
        bmi270_register_write(sensor, BMI270_PWR_CONF_REG, pwrConfigVal);
    }
    else
    {
        // Set the third bit (bit 2) of pwrConfigVal to 0 to disable the accelerometer
...
bool bmi270_EnableFifoSelfWakeup(BMI270 *sensor, bool enable)
{
    uint8_t pwrConfigVal; // create a variable to hold the power configuration value
...
{
  if (!bmi270_register_read(sensor, BMI270_CHIP_ID_REG, statusMessage, 1))
  {
    return false;
  }
  
  *statusMessage &= 0b00001111; 
...
{
    ret_code_t err_code;
    bmi270_register_write(sensor, BMI270_PWR_CONF_REG, 0x00);
    nrf_delay_us(450);
    ...
    bmi270_register_write(sensor, BMI270_PWR_CTRL_REG, 0x0E);
    bmi270_register_write(sensor, BMI270_ACC_CONF_REG, 0xA8);
    bmi270_register_write(sensor, BMI270_GYR_CONF_REG, 0xE9);
    bmi270_register_write(sensor, BMI270_PWR_CONF_REG, 0x02);
